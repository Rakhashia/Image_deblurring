                    %% GUI for deblurring images
                    %% Author: Nikhar P. Rakhashia
%% This is the initialization code 
% The following lines are written by the author himself:
% 1-10,43-54,57-112,115-177,180-447,449-457
% Lines 11-42 and 458-499 got generated during GUI creation
% Lines 55,56,113,114,178,179,448,449 are for creating a dialog
...
blurred_fft_r=my_dft2(blurred_img_r,[size_blurred_img(1), size_blurred_img(2)]); %FFT of red channel

% Green Channel of blurred image
blurred_img_g=blurred_img(:,:,2); %Green
blurred_fft_g=my_dft2(blurred_img_g,[size_blurred_img(1),size_blurred_img(2)]); %FFT of green channel

% Blue Channel of blurred image
blurred_img_b=blurred_img(:,:,3); %Blue
blurred_fft_b=my_dft2(blurred_img_b,[size_blurred_img(1),size_blurred_img(2)]); %FFT of blue channel
...
kernel_fft=my_dft2(kernel,[size_blurred_img(1),size_blurred_img(2)]); %FFT of kernel
...
recovered_img_r=real(my_idft2(inverse_filter_r)); % Red channel
recovered_img_g=real(my_idft2(inverse_filter_g)); % Green channel
recovered_img_b=real(my_idft2(inverse_filter_b)); % Blue channel 
...
reconstructed_img_butterworth_r=real(my_idft2(inverse_filter_r.*butterworthf)); %Reconstructed image
reconstructed_img_butterworth_g=real(my_idft2(inverse_filter_g.*butterworthf)); %Reconstructed image
reconstructed_img_butterworth_b=real(my_idft2(inverse_filter_b.*butterworthf)); %Reconstructed image
...
P_fft=my_dft2(P,[size_blurred_img(1),size_blurred_img(2)]); %FFT of P
...
%Outputs
cl_filtered_output(:,:,1)=real(my_idft2(cl_filter_r));
cl_filtered_output(:,:,2)=real(my_idft2(cl_filter_g));
cl_filtered_output(:,:,3)=real(my_idft2(cl_filter_b));
...
kernel_fft=my_dft2(kernel_temp,[size_blurred_img(1),size_blurred_img(2)]); %FFT of kernel
kernel_mag_squared=(abs(kernel_fft)).^2; %Magnitude square of kernel fft
...
%Output
reconstructed_img(:,:,1)=real(my_idft2(weiner_filter_r));
reconstructed_img(:,:,2)=real(my_idft2(weiner_filter_g));
reconstructed_img(:,:,3)=real(my_idft2(weiner_filter_b));
